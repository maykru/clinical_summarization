...
def main():
    ...
    target_path = 'data/MIMIC-III/target_population/cohort_2000'
    # maybe adjust paths
    ...
    # admission_id_list = icu_df['HADM_ID'].to_list()
    # sample_ids = [100486, 100566, 101813, 101952, 102230, 102432, 102667, 103040, 103309, 103825]
    admission_ids = []

    # for admission_id in tqdm(admission_id_list):
    # for i in range(1,5):
        # batch_ids = icu_df[icu_df['BATCH'] == i]['HADM_ID'].to_list()
    ...
    for admission_id in tqdm(admission_ids):
        ...
        combined_tl_rel = combined_tl_rel.dropna(subset=['TIME'])                   # remove entries without a
        ...
        formatted_last_day.to_csv(f'{target_dir}/24h_all_abs_{admission_id}.csv', index=False)
